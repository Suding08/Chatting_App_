...
reader.onload = async()=>{
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      <div className='flex items-center justify-between border-gray-600 text-gray-300 border-2 w-5/6 max-w-2xl backdrop-blur-2xl max-sm:flex-col-reverse rounded-lg'>
      <form onSubmit={handleSubmit}
     className='flex flex-col gap-5 p-10 flex-1'>
        <h3 className='text-lg'>Profile details</h3>
        <label htmlFor="avtar" className='flex items-center gap-3 cursor-pointer'>
          <input onChange={(e)=>setSelectedImg(e.target.files[0])} type="file" id='avtar' accept='image/png image/jpg image/jpeg' hidden/>
